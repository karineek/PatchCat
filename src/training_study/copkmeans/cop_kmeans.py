...
def cop_kmeans(dataset, k, ml=[], cl=[],
    ...
    if init_centers is not None:
        centers = np.asarray(init_centers, dtype=float)
        assert centers.shape[0] == k, "init_centers must have k rows"
    else:
...
def cop_predict_using_original_code(centers, embeddings):
    labels = []
    for x in embeddings:
        indices, distances = closest_clusters(centers, x)
        labels.append(indices[0])  # nearest center
